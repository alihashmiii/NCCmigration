...
function [res] = chemotaxis_pdedef0(npts, npde, t, x, y, u, ut, ux, uy, uxx, uxy, uyy)
...
spillRate = param.spillRate;
degRate = param.degRate;
...
% faster -- LJS
bigEat = (eatRate*u(:,ones(length(xcell),1)) - spillRate)/(eatWidth^2*2*pi).*exp(-1/2/eatWidth^2.*(...

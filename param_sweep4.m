...
numRepeats = 80; % number of runs per parameter combination, to gather stats
input = cell(numRepeats,1);

dateString = '2013_08_02'; % for the folder name to save all results in, even if simulations actually run over multiple days or are repeated on another day

totalTime = tic;
for repCtr = 21:(20+numRepeats) % run additional repeats for reference set to build up stats
    if isempty(dir(['results/parameterSweeps/' dateString '-allResults/*Run_' num2str(repCtr) '.mat'])) % check if this run hasn't been done, if previous sweeps have been aborted
        input{repCtr-20}.time = 18; % duration of each run, simulation time in hours
        
        % run model for reference set
        input{repCtr-20}.saveInfo = ['parameterSweeps/' dateString '-allResults/' datestr(now,'yyyy_mm_dd-HH_MM') ...

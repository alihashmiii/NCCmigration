...
sensAccColors = jet(length(sensingAccuracyValues));
    ...
    for needNeighboursCtr = 1:length(needNeighboursValues)
        ...
        for repCtr = 1:numRepeats
            ...
            try % sometimes we get corrupt files, which crashes the script
                ...
                experiment36singleLeader; % recreate the missing results file
        ...
        title(['Exp3.6: sensAcc=' num2str(sensingAccuracy) ', needNbrs=' num2str(needNeighbours)])
        
        % save plot
        filename = ['results/experiment36/figures/exp36'...
            '_sensingAcc_' num2str(sensingAccuracy) '_needNeighbours_' num2str(needNeighbours) ...
            '_allRuns'];
        ...
        subplot(length(sensingAccuracyValues),length(needNeighboursValues),length(needNeighboursValues)*(sensAccCtr - 1) + needNeighboursCtr)
        plot_migration_profile
        % xlabel('x/\mum'), ylabel(AX(1),'N(cells)'), ylabel(AX(2),'C(chemoattractant)')
        ...
        if sensAccCtr==1, hold on, end
        plot(xBins,squeeze(mean(sum(cellDistributions(paramCtr,:,:,:),3),2)),'Color',sensAccColors(sensAccCtr,:));
        if sensAccCtr==length(sensingAccuracyValues)
            xlabel('x/\mum'), ylabel('N(cells)'), legend(num2str(actualLeaderFraction((length(needNeighboursValues):length(needNeighboursValues):length(needNeighboursValues)*length(sensingAccuracyValues)) - length(needNeighboursValues)*length(sensingAccuracyValues) + paramCtr),precision))
        ...
        if sensAccCtr ==1, hold on, end
        plot(1:length(neighbours(paramCtr).numbers),neighbours(paramCtr).numbers./numRepeats,'Color',sensAccColors(sensAccCtr,:))
        if sensAccCtr==length(sensingAccuracyValues)
        ...
        if sensAccCtr ==1, hold on, end
        plot(neighbours(paramCtr).distancesBinEdges, neighbours(paramCtr).distances./numRepeats,'Color',sensAccColors(sensAccCtr,:))
        if sensAccCtr==length(sensingAccuracyValues)
            title(['Exp3.6: sensAcc=' num2str(sensingAccuracy) ', needNbrs=' num2str(needNeighbours)])
        ...
        if sensAccCtr ==1, hold on, end
        plot(neighbours(paramCtr).areasBinEdges, neighbours(paramCtr).areas./numRepeats,'Color',sensAccColors(sensAccCtr,:))
        if sensAccCtr==length(sensingAccuracyValues)
            xlabel('area/\mum^2'), ylabel('N(cells)'), legend(num2str(actualLeaderFraction((length(needNeighboursValues):length(needNeighboursValues):length(needNeighboursValues)*length(sensingAccuracyValues)) - length(needNeighboursValues)*length(sensingAccuracyValues) + paramCtr),precision))
...
filename = ['results/experiment36/figures/exp36_migrationProfiles'];
...
filename = ['results/experiment36/figures/exp36_sensingAcc_migrationProfiles2gether'];
...
filename = ['results/experiment36/figures/exp36_sensingAcc_neighbourRelations'];
exportfig(neighbourRelationsFig,[filename '.eps'],exportOptions);
system(['epstopdf ' filename '.eps']);
system(['cp ' filename '.pdf results/PDFs/' filename '.pdf']); % copying finished plots to a place where Dropbox will sync them
close(neighbourRelationsFig);

save('results/experiment36/figures/experiment36collatedResults','xBins','cellDistributions','caDistribution','actualLeaderFraction','sensingAccuracies','neighboursNeeds')
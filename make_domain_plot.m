...
L0_values = 250:5:350;
t_s_values = 10:20;
sum_error_squared = NaN(length(a_values),length(t_s_values),length(L0_values),length(Linf_values));
for l = 1:length(L0_values)
    ...
    for m = 1:length(Linf_values)
        ...
        for i = 1:length(a_values)
            a = a_values(i);
            for j = 1:length(t_s_values)
                t_s = t_s_values(j);
                [~, estimatedLengths, ~] = domain_growth([],time,0,Linf,a,L0,t_s);
...
title('\alpha')
subplot(2,2,2)
plot(t_s_values,sum_error_squared(ind(1),:,ind(3),ind(4)))
title('t_s')
subplot(2,2,3)
...
figure
a = a_values(ind(1))
t_s = t_s_values(ind(2))
Linf = Linf_values(ind(4))
L0 = L0_values(ind(3))
 %% domain growth %%
[~, domainLengths, ~] = domain_growth([],t,0,Linf,a,L0,t_s);
hold on

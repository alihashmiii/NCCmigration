...
%         nx = int64(x_length);
    ...
    if insert==1
        load avi_mat/xlat_new
        xmin = min(xlat_new);
        nx = int32(length(xlat_new));
%         nx = int32(x_length);
        ny = int32(y_length);
    else
        nx = int32(50); % number of x grid points including boundary
        ny = int32(50);
    end
end
...
if isunix==1
    ...
    opti = zeros(4,1,'int32'); % default integrator options
    opti(1) = int32(3); % max num grid levels
    opti(2) = int32(20); % max Jacobian evaluations
    opti(3) = int32(10);    % max newton iterations
    opti(4) = int32(100);   % max linear equation iterations
    optr = [1.0;1.0;1.0];   % specifies parameters in the space and time monitors
    if insert==1
        itrace = int32(0);
    else
        itrace = int32(0);     % level of trace information
    end
end
% rwk = zeros(188000,1);

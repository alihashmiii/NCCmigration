        ...
        for slowSpeedCtr = 1:length(slowSpeeds)
            ...
            for insertEveryStepsCtr = 1:numInsertValues
                ...
                for repCtr = 1:numReps
                    ...
                    refCellFractions(insertEveryStepsCtr,repCtr) = ...
                        nnz(cells(1,:)<=max(out.xlat_save{timeIdx})/3)/size(cells,2);
                    % calc stream width
                    refStreamWidths(insertEveryStepsCtr,repCtr) = ...
                        std(cells(2,cells(1,:)<=max(out.xlat_save{timeIdx})/3));
                end
                %% load DAN data
                for repCtr = 1:numReps
                    if insertEverySteps==6
...
%             errorbar(danZoneFig.Children,60./insertEveryStepsValues,mean(cellFractions,2),std(cellFractions,0,2)/sqrt(numReps))
            set(0,'CurrentFigure',danZoneFig)
            notBoxPlot(cellFractions',60./insertEveryStepsValues)
...
%             xlim(danZoneFig.Children,[1 10])
            ...
            danZoneFig.Children.YLim = [0.75, 1];

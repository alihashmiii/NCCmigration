...
function out = CA7(in,experiment)
...
tic
%% Model Type Inputs %%
param.growingDomain = 0;     % the domain grows
...
sensingAccuracy = 0.001; % relative accuracy with which concentration can be measurem. dC/C has to be greater than this to be noticed. This is the baseline value for the starting concentration, scales with 1/sqrt(c) -- LJS
...
%% caSolve parameters %%
param.diffus = 1e5;%252e3;    % chemoattractant diffusivity (in (mu)^2/h), for VEGF diffusing in the matrix this should probably be around 7e-11m^2/s = 252e3(mu)^2/h, for membrane bound VEGF unknown/near zero -- LJS
param.chi = 0;                  % chemoattractant production term (usually 0.0001)
param.eatRate = 0;                      % chemoattractant consumption rate
param.spillRate = 1e3;
param.degRate = 1e5;
param.eatWidth = cellRadius/2;         % width of eating chemoattractant, equivalent to gaussian sigma
...
param.t_s = -16; %parameter used in domain_growth
%%
...
for timeCtr=1:numTsteps
    ...
    if caSolve==1
        ...
        if ((param.experiment==1)||(param.experiment==2))&&(in.it==2)&&(t_save(timeCtr)==in.changeTime)
            temp = chemotaxis_solve0(t_save(timeCtr),t_save(timeCtr+1),ind,iwk,rwk,cells_in,param.initialDomainLength,param.domainHeight,length(xlat_new),length(ylat_new),param.insert);
        elseif timeCtr>1
            temp = chemotaxis_solve0(t_save(timeCtr),t_save(timeCtr+1),ind,iwk,rwk,cells_in,param.initialDomainLength,param.domainHeight,length(xlat_new),length(ylat_save{timeCtr-1}),param.insert);
        else
            temp = chemotaxis_solve0(t_save(timeCtr),t_save(timeCtr+1),ind,iwk,rwk,cells_in,param.initialDomainLength,param.domainHeight,length(xlat_new),32,param.insert);
        end

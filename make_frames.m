...
for j=1:length(frames2show)
    ...
    make_plot(cells_save{k},cellsFollow{k},xlat_save{k},ylat_save{k},ca_save{k},filopodia_save{k},numFilopodia,attach_save{k},cellRadius,filolength,sensingAccuracy,showColorbar,caCmap,0)
    if growingDomain==0
        xlim([0,300])
    end
    title(['Cell migration at time t = ',mat2str(t_save(k)),' hours with ' num2str(size(cells_save{k},2)) ' cells and ' num2str(min([size(cells_save{k},2) nnz(cellsFollow{k}==0)])) ' leaders.'])

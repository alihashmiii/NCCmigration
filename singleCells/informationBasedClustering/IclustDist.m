...
if size(S,1)~=size(S,2)
    error ('S must be a squared matrix')
end

if min(min(S))<0
    fprintf ('The DISTORTION matrix includes NEGATIVE values -- do you wish to continue? (press any key)\n');
    pause
end

if any(S-S')
    fprintf ('NON SYMMETRIC distortion matrix -- abort\n');
...
minS = min(S);
diagS = diag(S)';
if any(find(minS < diagS))
    fprintf ('DIAGONAL values are not MINIMAL in S - do you wish to continue? (press any key)\n');    
    pause
    warning ('Non minimal diagonal values can lead to unreliable output!')
end
...
prm.LoopLimit = 500;
prm.LocalEps = 1e-10;
prm.SmallChange = prm.LocalEps*10;
...
% if there's no convergence warning, make an empty one, to have conistent
% fields in output structure -- LJS
if ~isfield(tmpC,'WARNING')
    tmpC.WARNING = 'no warning';
end
